...
class Renderoija:
    ...
    def _piirra_varien_selitteet(self, algoritmi):
        self._ikkuna.blit(self._fontti.render("lähtö", True, PUNAINEN), (100,1040))
        self._ikkuna.blit(self._fontti.render("maali", True, VIHREA), (200,1040))
        #self._ikkuna.blit(self._fontti.render("vierailtu", True, TURKOOSI), (300,1040))
        self._ikkuna.blit(self._fontti.render("jonossa", True, KELTAINEN), (300,1040))
        self._ikkuna.blit(self._fontti.render("reitti", True, SININEN), (420,1040))
        if algoritmi == "JPS":
            self._ikkuna.blit(self._fontti.render("jump point", True, PINKKI), (520,1040))
        
    def _piirra_valittu_algo(self, algoritmi):
        self._ikkuna.blit(self._fontti.render(f'Valittu {algoritmi.nimi}', True, VALKOINEN), (1200,300))

    def _piirra_alkupisteen_tiedot(self, alku):
        alkupiste = self._fontti.render(f'Alku     Y: {alku.y},  X: {alku.x}', True, VALKOINEN)
        self._ikkuna.blit(alkupiste, (1200, 700))
        
    def _piirra_loppupisteen_tiedot(self, loppu):
        loppupiste = self._fontti.render(f'Loppu   Y: {loppu.y},  X: {loppu.x}', True, VALKOINEN)
    ...
    def _piirra_kayttoohje(self, animoitu):
        ...
        animaatio = self._fontti.render(f'Animaatio (A): {animoitu}', True, VALKOINEN)
